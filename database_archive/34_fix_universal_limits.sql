-- =========================================================================
-- FUNCTION: rpc_quote_vacancy_price (V7 - Exploit Prevention)
-- =========================================================================
...
BEGIN
    ...
    ORDER BY (slug ILIKE v_profile_plan_raw) DESC
    LIMIT 1;
    ...
    -- 3. CALCULAR CICLO DE USO
    v_usage_start_date := COALESCE(v_plan_expires_at - interval '30 days', date_trunc('month', now()));
    
    -- 4. CONTAR USO REAL (SSOT y Anti-Exploit)
    -- 🛡️ SENIOR FIX: Prevenir que al eliminar una vacante que ya fue 'cerrada' o 'finalizada' (que tiene closed_at o contratado) 
    -- se le devuelva el cupo a la empresa.
    ...
    WHERE empresa_id = p_empresa_id 
      ...
      AND (
          status != 'eliminada' 
          OR (status = 'eliminada' AND (closed_at IS NOT NULL OR contratado_id IS NOT NULL))
      )
      AND created_at >= v_usage_start_date;

    -- 5. EXTRACCIÓN UNIVERSAL DE BENEFICIOS (SSOT)
    ...
    -- 🛠️ HARDCODE FALLBACKS SI LA BD ESTÁ INCOMPLETA (Protege a TODOOOOOOOS los planes)
    IF v_plan_slug ILIKE '%micro%' AND v_fixed_limit = 0 THEN v_fixed_limit := 7; END IF;
    IF v_plan_slug ILIKE '%pro%' AND v_fixed_limit = 0 THEN v_fixed_limit := 30; END IF;
    IF v_plan_slug ILIKE '%basic%' AND v_fixed_limit > 0 THEN v_fixed_limit := 0; END IF;
