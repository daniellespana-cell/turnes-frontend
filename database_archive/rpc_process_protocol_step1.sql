...
BEGIN
    ...
    -- El check_constraint de la BD requiere que tipo sea ('INGRESO', 'RETIRO', 'DEPOSITO', 'PAGO_SERVICIO', etc).
    INSERT INTO public.movimientos (billetera_id, tipo, monto, estado, concepto, referencia)
    VALUES (v_user_id, 'RETIRO', p_amount, 'completado', p_concept, 'STEP1_' || extract(epoch from now()))
