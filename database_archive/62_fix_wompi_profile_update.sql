-- 🛡️ 62_fix_wompi_profile_update.sql
-- Restaura la actualización del perfil que se perdió durante la refactorización de los locks.
...
DECLARE
    ...
    v_type_code TEXT;
    v_item_type TEXT;
    v_item_id TEXT; -- Añadimos v_item_id para los planes
...
BEGIN
    ...
    -- 4. 🎯 PARSER DE UUID Y EXTRACCIÓN DE ITEM_ID A PRUEBA DE BALAS
    BEGIN
        IF v_reference LIKE 'REF-%' THEN
            ...
            -- Extraer el slug del plan (ej: REF-uuid-S-pro-12345)
            v_item_id := split_part(v_reference, '-', 8); 
        ELSE
            v_user_id := LEFT(v_reference, 36)::UUID;
            v_type_code := substring(v_reference from 38 for 1);
            -- Extraer el slug del plan (ej: uuid-S-pro-12345)
            v_item_id := split_part(v_reference, '-', 7); 
        END IF;
    ...
    -- 6. 💰 ENRUTAMIENTO ATÓMICO DEL DINERO Y LA ENTREGA
    ...
    ELSIF v_item_type = 'plan' THEN
        -- 🛠️ FIX: Actualizar el perfil del usuario!
        -- Si v_item_id no vino (formato legacy), asumimos un fallback o fallamos seguro
        IF v_item_id IS NULL OR v_item_id = '' THEN
             v_item_id := 'pro'; -- Default de seguridad o rechazar
        END IF;
        
        UPDATE public.perfiles 
        SET plan = v_item_id, plan_expires_at = now() + interval '30 days', updated_at = now() 
        WHERE id = v_user_id;
