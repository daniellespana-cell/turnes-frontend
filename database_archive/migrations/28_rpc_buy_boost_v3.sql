-- 🛠️ 28_rpc_buy_boost_v3.sql
-- OBJETIVO: Compra de Impulso Urgente (48H) con validación de saldo.
-- FIX 1: La columna 'saldo' vive en 'billeteras', no en 'perfiles'.
-- FIX 2: La tabla de historial financiero es 'movimientos', no 'transacciones'.
...
BEGIN
    ...
    -- B. Verificar saldo (Billetera + Bloqueo de fila)
    ...
    -- E. REGISTRAR MOVIMIENTO (Historial Real)
    -- Estructura de la tabla 'movimientos' detectada en migraciones
    INSERT INTO public.movimientos (user_id, tipo, monto, descripcion, metadata)
    VALUES (
        v_user_id, 
        'DEBITO', 
        p_price, 
