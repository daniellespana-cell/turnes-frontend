-- 🛠️ 28_rpc_buy_boost_v2.sql
-- OBJETIVO: Compra de Impulso Urgente (48H) con validación de saldo.
-- FIX: La columna 'saldo' vive en 'billeteras', no en 'perfiles'.
...
BEGIN
    ...
    -- B. Verificar saldo (La tabla correcta es 'billeteras')
    -- Usamos FOR UPDATE para evitar race conditions en el saldo
    ...
    -- E. REGISTRAR TRANSACCIÓN PARA HISTORIAL
    INSERT INTO public.transacciones (user_id, tipo, monto, descripcion, metadata)
    VALUES (
        v_user_id, 
        'DEBITO', 
        p_price, 
        'Activación de Impulso Urgente: ' || v_vacancy_title,
        jsonb_build_object(
            'vacancy_id', p_vacancy_id,
            'duration_hours', 48,
            'system', 'Turnes Boost Engine'
        )
    );
