...
BEGIN
    ...
    -- 2. BLOQUEO DE INTEGRIDAD (Opcional, pero se mantiene por seguridad de concurrencia)
    PERFORM 1 FROM public.perfiles WHERE id = v_empresa_id FOR UPDATE;

    -- 3. COTIZACIÓN REAL (Calculamos pero NO COBRAMOS)
    -- Invocamos el RPC de cotización solo para fines informativos o registros futuros
    ...
    -- 4. VALIDACIÓN FINANCIERA ELIMINADA (V6 - Publicación Libre)
    -- El cobro de comisiones y uso de saldo se delega al paso de Contratación (ContactSidebar).
    -- La vacante se inserta directamente, independientemente de v_total_cost.

    -- 5. INSERCIÓN DE LA VACANTE
