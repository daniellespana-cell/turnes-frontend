...
const reactRouterDomExports = ['Link', 'NavLink', 'Outlet', 'useNavigate', 'useLocation', 'useParams', 'useSearchParams', 'RouterProvider', 'Navigate'];
...
for (const [filePath, missingVars] of Object.entries(missingVarsMap)) {
    ...
    for (const v of missingVars) {
        // VERIFICAR SI YA ESTÁ DECLARADA EN EL ARCHIVO
        const regex = new RegExp(`\\b${v}\\b`);
        // Si el archivo ya tiene una importación o declaración del símbolo, no lo agregamos.
        // Específicamente buscamos imports.
        const importRegex = new RegExp(`import\\s+[^;]*\\b${v}\\b[^;]*;`, 's');
    ...
    if (importStatements) {
        // En lugar de agregar siempre arriba, primero limpiamos las líneas que pudimos haber agregado antes
...
console.log(`Se han re-agregado importaciones a ${fixedCount} archivos (sin duplicados).`);
