            ...
            <div className={`bg-zinc-950 border rounded-2xl p-5 space-y-6 relative overflow-hidden transition-all ${isSealed ? 'border-blue-500/10' : 'border-white/5'
                ...
                <div className="flex items-center gap-3 relative z-10">
                    <div className="relative">
                        <img
                            src={AssetResolver.getAvatar(candidate?.avatar) || 'https://via.placeholder.com/150'}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1">
                            <span className="text-[7px] font-black text-zinc-700 uppercase tracking-widest">
                                {isEmpresa ? 'Plan Activo' : 'Estado Perfil'}
                            </span>
                            <p className={`text-[10px] font-bold uppercase ${isEmpresa && config.plan === 'Pro' ? 'text-pink-500' : 'text-emerald-600'}`}>
                                {isEmpresa ? config.plan : 'VERIFICADO'}
                            </p>
                            {isEmpresa && config.beneficioPlan && (
                                <div className="mt-1 py-0.5 px-1.5 bg-white/5 border border-white/10 rounded-md inline-block">
                                    <span className="text-[6px] font-black text-white/50 uppercase tracking-tighter italic">
                                        {config.beneficioPlan}
                                    </span>
                                </div>
                            )}
                        </div>
                        <div className="space-y-0.5 text-right">
                            <span className="text-[7px] font-black text-zinc-700 uppercase tracking-widest">
                                {isEmpresa 
                                    ? (config.isFijo ? 'Cargo Único Turnes' : 'Comisión Turnes')
                                    : 'Protocolo Seguridad'}
                            </span>
                            <p className={`text-[14px] font-black ${isEmpresa && config.cargo === 0 ? 'text-emerald-400' : 'text-white'}`}>
                                {isEmpresa 
                                    ? (config.cargo === 0 ? 'SIN COSTO' : `$${config.cargo.toLocaleString()}`)
                                    : 'BLINDADO'}
                            </p>
                        </div>
                    </div>
