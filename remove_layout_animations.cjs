...
const transformPath = path.resolve(__dirname, 'transform_layout.cjs');
  ...
  root.find(j.JSXAttribute).forEach(attrPath => {
    ...
    if (targetAttributes.includes(attrName) && attrPath.node.value && attrPath.node.value.type === 'JSXExpressionContainer') {
      ...
      if (expression.type === 'ObjectExpression') {
        ...
        if (heightRemoved && !hasScaleOrScaleY) {
            // Optional: inject scaleY or scale? Let's just remove height, it's safer.
        }
...
fs.writeFileSync(transformPath, transformCode);

const files = glob.sync('src/**/*.jsx');

run(transformPath, files, {
  verbose: 1,
  dry: false,
  print: false,
  babel: true,
  ...
  console.log('Codemod completo para layout animations.');
