...
const AppNavbar = ({ user, isSidebarExpanded, onOpenMobileSidebar }) => {
  ...
  // user.role: 'empresa' | 'postulante'  (set by AuthContext.normalizeUser via profileData.rol)
  // user.plan:  'Básico' | 'Micro' | 'Pro'  (set by AuthContext.normalizeUser via PLANS_CONFIG, only for 'empresa')
  // Postulantes never have user.plan set — so the second condition is naturally false for them.
  ...
  // Message badge: count unread CHAT_MESSAGE notifications
  const { notifications } = useNotificationsContext();
  const unreadMessages = notifications.filter(n => n.tipo === 'CHAT_MESSAGE' && !n.leida).length;
  ...
  return (
    <nav className="fixed top-0 left-0 w-full z-50 bg-[#060606]/85 backdrop-blur-2xl border-b border-white/[0.04] transition-all duration-500">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20 relative gap-2">
          ...
          {/* 2. Logo Section */}
          <Link to="/dashboard" className="hidden md:block flex-shrink-0 transition-transform hover:scale-[1.02] active:scale-[0.98]">
            <img src={turnesLogo} alt="Turnes" className="h-8 w-auto object-contain brightness-110" />
          </Link>
          ...
          <div className="flex items-center gap-1 md:gap-4 relative flex-none justify-end">
            ...
            {/* ✦ RECHARGE BUTTON — Only for Empresa, globally visible */}
            {isBusiness && (
              <RechargeButton className="h-9 px-0 md:px-4 text-[10px]" />
            )}

            {/* ✦ PREMIUM UPGRADE BUTTON — Only for Empresa on Basic plan */}
            {showUpgradeButton && (
              <button
              ...
              >
                {/* — MOBILE: Círculo icon-only con anillo de pulso ámbar — */}
