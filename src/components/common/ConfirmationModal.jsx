...
const ConfirmationModal = ({ isOpen, onClose, onConfirm, title, message, type = 'delete', confirmText = 'Confirmar' }) => {
    ...
    return createPortal(
        <div className={`fixed inset-0 z-50 flex items-center justify-center p-4 transition-all duration-300 ${isOpen ? 'bg-black/80 backdrop-blur-sm' : 'bg-black/0 backdrop-blur-none pointer-events-none'}`}>
            <div
                className={`bg-[#0a0a0a] border border-white/10 w-full max-w-sm rounded-[2rem] p-8 shadow-2xl transform transition-all duration-300 relative overflow-hidden ${isOpen ? 'scale-100 opacity-100 translate-y-0' : 'scale-95 opacity-0 translate-y-4'}`}
            >
                ...
                <div className="relative z-10 flex flex-col items-center text-center">
                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-6 border ${type === 'delete' ? 'bg-red-500/10 border-red-500/20 text-red-500' : 'bg-zinc-800 border-white/5 text-zinc-400'}`}>
                        {type === 'delete' ? <Trash2 size={20} /> : <Copy size={20} />}
                    </div>
