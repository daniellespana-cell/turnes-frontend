...
import { Layers, CheckCircle2 } from 'lucide-react'; // Iconos técnicos
import CandidatoCard from './CandidatoCard';
import CandidatoRow from './CandidatoRow';

const CandidatosContent = ({ activeTab, pendientes, historial, onUpdate, onSellar }) => {
  const isPendientes = activeTab === 'pendientes';
  const data = isPendientes ? pendientes : historial;

  // LOG DE CONTROL
  useEffect(() => {
    console.log(`[Turnes System] Tab: ${activeTab.toUpperCase()} | Registros: ${data.length}`);
  }, [data, activeTab]);
  ...
  if (data.length === 0) {
    return (
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        className="w-full h-[400px] rounded-3xl border border-white/5 bg-[#09090b] relative overflow-hidden flex flex-col items-center justify-center"
      >
